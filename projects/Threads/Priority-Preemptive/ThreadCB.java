...
public class ThreadCB extends IflThreadCB 
{
    ...
    */
    static public ThreadCB do_create(TaskCB task)
    {
        ...
        int randomnm = minimum + (int)(Math.random() * maximum);
        ...
        newthread.setPriority(randomnm);
    ...
    */
    ...
    {
		ThreadCB maxpriorityThread=null;
		int j=0;
        ThreadCB oldThread=null,newThread;
        if (!rdQueue.isEmpty())
        {    
          maxpriorityThread=rdQueue.get(0);
	      int i= -1;
          for(ThreadCB itr : rdQueue)
          {
			i++;
            if (itr.getPriority() < maxpriorityThread.getPriority())
            {
                maxpriorityThread=itr;
				j=i;
            }
          }
        }
        ...
        try
        {
			...
			if(! rdQueue.isEmpty())
		    {
             if(maxpriorityThread.getPriority()<oldThread.getPriority())
             {
               oldThread.setStatus(ThreadReady);
               rdQueue.add(oldThread);
