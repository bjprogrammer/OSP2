...
public class Device extends IflDevice
{
    ...
    */
    public IORB do_dequeueIORB()
    {
    	...
    	{
			p = iorbQueue.length() - 1;
			for(int i = iorbQueue.length() - 1; i >= 0; i--) 
    	    {
    		 IORB currentiorb = (IORB) ((GenericList) iorbQueue).getAt(i);
			 IORB selectediorb=(IORB) ((GenericList) iorbQueue).getAt(p);
			 if(Math.abs(currentiorb.getCylinder()-lastcylinder)< Math.abs(selectediorb.getCylinder()-lastcylinder))
			 {
				 p=i;
		     }
		    }
			
    		IORB iorb = (IORB) ((GenericList)iorbQueue).getAt(p);
